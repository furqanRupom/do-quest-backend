...
export class SubmissionService {
    ...
    async createSubmission(createSubmissionDto:CreateSubmissionDto,taskId:string,userId:string): Promise<Submission> {
        ...
        if(task.status !== TaskStatus.active){
            throw new BadRequestException('Cannot submit to a task that is not active');
        }
        const submission = await this.submissionRepository.createSubmission(createSubmissionDto,taskId,userId);
        return submission;
    }

    async approveSubmission(approveSubmissionDto: ApproveOrRejectDto) : Promise<{ capturedAmount: number; winnerSubmissionId: string }> {
        ...
        if (!task || task.user.toString() !== approverId) {
            throw new ForbiddenException('Not authorized to approve submissions for this task');
        }
        if (task.paymentIntentId == null) {
            throw new BadRequestException('No payment intent associated with this task');
        }
    ...
    async rejectSubmission(approveSubmissionDto: ApproveOrRejectDto): Promise<void> {
        const { taskId, submissionId, approverId } = approveSubmissionDto;
        const task = await this.tasksRepository.findTaskById(taskId);
        if (!task || task.user.toString() !== approverId) {
            throw new ForbiddenException('Not authorized to reject submissions for this task');
        }
